...
def tools_fit(So,n_model,minK,maxK,minDH,maxDH,minq0,maxq0):
  nconc = len(So)
  ndim = nconc*4+n_model+1
  bo = [(minDH,maxDH)]
  lab = [r'$\Delta H_{\rm bind}$']
...
def theta_func(Xtot,Kv,Ptot):
  xf =  Xfree(Xtot,Kv,Ptot)
  z = Z(xf,Kv)
  n = len(Kv)
  out = 0
  for i in range(n):
    out = out + (i+1)*xf**(i+1)*Kv[i]
  return out*(1.0/z)
...
def Best_fit(Pmatrix,Xmatrix,DHmatrix,Vc,vi,So,bo,dim,nw,nc,lab,nmodel,eArgs):
  ...
  def Nlogprob(p):
    ...
    logKv = p[1:1+nmodel]
    Kv = 10**logKv
    s = p[1+nmodel:1+nmodel+nconc]
    CS = p[1+nmodel+nconc:1+nmodel+2*nconc]
    eps = p[1+nmodel+2*nconc:1+nmodel+3*nconc]
    q0 = p[1+nmodel+3*nconc:1+nmodel+4*nconc]
