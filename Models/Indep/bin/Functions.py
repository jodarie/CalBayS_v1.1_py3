...
def Xfree(Xtot,n,K,Ptot):
  try:
    res = (((Xtot-n*Ptot)*K-1.0)/(2.0*K))+(np.sqrt(((n*Ptot-Xtot)*K+1.0)**2.0+4.0*K*Xtot)/(2.0*K))
    if 0<res<Xtot:
      return res
    else:
      return Xtot
...
def theta_func(Xtot,n,K,Ptot):
  xf =  Xfree(Xtot,n,K,Ptot)
  return n*(xf)*(K/(1.0+xf*K))

def Best_fit(Pmatrix,Xmatrix,DHmatrix,Vc,vi,So,bo,dim,nw,nc,lab,nArgs,eArgs):
  nconc = len(So)
  DH_exp = DHmatrix[1:,:]
  Ndata = len(DH_exp)
  def Nlogprob(p):
    ...
    if nArgs[0]:
      NProb = -ln_Gauss(n,nArgs[1],nArgs[2])
    else:
      NProb = 0
